<properties 
    pageTitle="Rakendamise streaming stsenaariumi Tõrkesiirde | Microsoft Azure'i" 
    description="Selles teemas kirjeldatakse, kuidas rakendada Tõrkesiirde streaming stsenaariumi." 
...
#<a name="implementing-failover-streaming-scenario"></a>Rakendamise Tõrkesiirde streaming stsenaarium

Juhendis näitab, kuidas sisu (plekid) kopeerimine üks vara teise koondamise tellitav streaming käsitlemiseks. See stsenaarium on kasulik klientidele, mida soovite häälestada oma CDN abil Tõrkesiirde kaks andmekeskuste puhul on katkestuste ühel meie andmekeskuste vahel.
Juhendis kasutab Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST API ja Azure salvestusruumi SDK näidata järgmisi toiminguid.

1. "Andmekeskuse a" Media Services konto häälestamine.
1. Standard faili üleslaadimine rakendusse allika vara.
1. Vara kodeerida mitme bit rate MP4 failid. 
1. Kirjutuskaitstud SAS locator on lugemisõigus ümbris salvestusruumi konto, mis on seotud andmeallika varade vara andmeallika loomine
1. Container source varade nime toomine kirjutuskaitstud SAS locator eelmises etapis loodud. Läheb vaja selle teabe kopeerimine plekid vahel salvestusruumi kontod (selgitatud teemas allpool.)
1. Saate luua ka origin locator kodeering tööülesande loodud vara. 

Seejärel, on tõrkesiire käsitlema.

1. "Andmete keskmist b" Media Services konto häälestamine.
1. Sihtrakenduse tühja varade meediumi teenusekonto sihtrakenduse loomine
1. Looge kirjutamine SAS locator target tühja vara soovitavas salvestusruumi konto, mis on seotud target varade ümbris juurdepääs olema.
1. Azure'i salvestusruumi SDK abil saate kopeerida plekid (varade failid) salvestusruumi lähtekonto "Andmekeskuse A" ja "Andmete keskmist B" target salvestusruumi konto vahel (need salvestusruumi kontod on seotud huvi vara).
1. Seostada target bloobimälu container target vara kopeeritud plekid (varade failid). 
1. Luua mõne origin locator "Andmete keskmist B" vara ja määrake locator Id, mis loodi vara "Andmekeskuse a". 
1. See annab teile streaming URL-id, kus URL-ide suhtelised teed on samad (ainult alus URL-id on erinevad). 
 
Seejärel katkestuste käsitlema saate luua CDN-ID need origin lokaatorid peal. 

Kehtivad järgmisega.

- Media Services SDK praegune versioon ei toeta loomise on locator määratud locator id-ga. Selles ülesandes saavutamiseks kasutame Media Services REST API-ga.
- Media Services SDK praegune versioon ei toeta programmiliselt loomisel IAssetFile teavet, mida soovite seostada vara varade failid. Selles ülesandes saavutamiseks kasutame CreateFileInfos Media Services REST API. 
- Krüptitud salvestusruumi varad (AssetCreationOptions.StorageEncrypted) pole toetatud kopeerimise (kuna krüptovõtme olla erinev, nii Media Servicesi kontod). 
- Kui soovite dünaamilise pakendit ära, esmalt saad vähemalt üks nõudmisel Streaming reserveeritud üksused. Lisateavet leiate teemast [Dünaamiliselt varade pakendit](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Kaaluge Media Servicesi [Paljundaja tööriista](http://replicator.codeplex.com/) alternatiivina rakendamise Tõrkesiirde streaming stsenaarium käsitsi. See tööriist võimaldab teil ise varad üle kahe Media Servicesi kontod.

##<a name="prerequisites"></a>Eeltingimused
 
- Kahe Media Servicesi kontod uude või olemasolevasse Azure'i tellimus. Vaadake, [Kuidas Media Services konto loomiseks](media-services-portal-create-account.md).
- Operatsioonisüsteemid: Windows 7, Windows 2008 R2 või Windows 8.
- .NET framework 4.5 või .NET Framework 4.
- Visual Studio 2010 SP1 või uuem versioon (Professional, Premium, Ultimate või Express).
 
##<a name="set-up-your-project"></a>Projekti häälestamine

Selles jaotises luua ja C# konsooli rakendus projekti häälestamine.

1. Visual Studio abil saate luua uue lahenduse, mis sisaldab C# konsooli rakendus projekti. Sisestage HandleRedundancyForOnDemandStreaming nimi ja seejärel klõpsake nuppu OK.
1. Looge kaust SupportFiles failina HandleRedundancyForOnDemandStreaming.csproj projekti samal tasemel. Kaustas SupportFiles OutputFiles ja MP4Files kaustu luua. Kopeerige MP4 faili MP4Files kausta (selles näites kasutatakse BigBuckBunny.mp4 fail). 
...
1. Lisage muud viited, mis on selle projekti jaoks vajaliku: System.Configuration, System.Runtime.Serialization ja System.Web mitu.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Lisage koondamise nõudmisel streaming koodi.



1. Saate lisada programmi klassi klassi taseme järgmised väljad.
...
1. Järgmise meetodi määratluse vaikimisi põhi asendamiseks tehke järgmist.
...
1. Meetodi määratlusi, mida nimetatakse kaudu põhi on määratletud allpool.
...
Nüüd saate ülemuse liikluse marsruutimiseks päringutele kahe andmekeskuste ja seega Tõrkesiirde puhul katkestuste vahel.
