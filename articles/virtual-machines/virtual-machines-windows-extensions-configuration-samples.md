<properties
   pageTitle="Valimi konfigureerimine Windows VM laiendid | Microsoft Azure'i"
   description="Valimi konfiguratsioon laiendiga mallide koostamine"
...
# <a name="azure-windows-vm-extension-configuration-samples"></a>Windows Azure'i VM laiend konfiguratsiooni näidised

> [AZURE.SELECTOR]
- [PowerShelli - Mall](virtual-machines-windows-extensions-configuration-samples.md)
- [CLI - Mall](virtual-machines-linux-extensions-configuration-samples.md)

<br>

Selles artiklis on toodud valimi konfiguratsiooni konfigureerida Azure VM laiendid Windows VMs.

Need laiendid kohta leiate lisateavet teemast [Azure VM laiendid ülevaade.](virtual-machines-windows-extensions-features.md)

Laiend mallide loomise kohta leiate lisateavet teemast [loome laiend mallide.](virtual-machines-windows-extensions-authoring-templates.md)

Selles artiklis on loetletud mõned Windowsi laiendid oodatud väärtused.

## <a name="sample-template-snippet-for-vm-extensions-with-iaas-vms"></a>Valimi malli koodilõigu VM laiendid koos IaaS VMs jaoks.
Malli koodilõigu juurutamiseks laiendid näeb välja järgmine:
...
## <a name="sample-template-snippet-for-vm-extensions-with-vm-scale-sets"></a>Valimi malli koodilõigu VM laiendid koos VM skaala komplekti jaoks.
...
Enne juurutamist laiendamine kontrollige laiend uusim versioon ja asendage "typeHandlerVersion" praegune uusim versioon.

Ülejäänud artikkel pakub valimi konfiguratsioone Windows VM laiendid.

Enne juurutamist laiendamine kontrollige laiend uusim versioon ja asendage "typeHandlerVersion" praegune uusim versioon.

### <a name="customscript-extension-14"></a>CustomScript laiend 1.4.
      {
...
#### <a name="parameter-description"></a>Parameetri kirjeldus:

- fileUris: komaga eraldatud loend URL-ide failid, mis laaditakse VM pikendamisega. Failid laaditakse alla, kui midagi on määratud. Kui failid on Azure Storage, on fileURLs saate privaatseks märgitud ja correspoding storageAccountName ja storageAccountKey saab edasi privaatne parameetrid nende failidele juurde pääseda.
- commandToExecute: [kohustuslik parameeter]: see on nupp, mis käivitatakse pikendamisega.
- storageAccountName: [valikuliste parameetrite]: juurdepääsuks fileURLs, kui need on märgitud eraviisiliseks Salvestusruumikonto nimi.
- storageAccountKey: [valikuliste parameetrite]: salvestusruumi konto võti juurdepääsuks fileURLs, kui need on märgitud eraviisiliseks.

### <a name="customscript-extension-17"></a>CustomScript laiend 1.7.

Vaadake CustomScript versioon 1.4 parameetri kirjelduse. Versiooni 1.7 pakub tuge saatmine skripti parameters(commandToExecute) protectedSettings, sel juhul need krüptitakse enne saatmist. "commandToExecute" parameeter saab määrata, kas sätted või protectedSettings, kuid mitte mõlemat.
...
### <a name="vmaccess-extension"></a>VMAccess laiendamine.
...
### <a name="dsc-extension"></a>DSC laiendamine.
      {
...
### <a name="trend-micro-deep-security-agent"></a>Trend Micro sügav turvalisus Agent.
      {
...
### <a name="vormertric-transparent-encryption-agent"></a>Vormertric läbipaistev krüptimise Agent.
            {
              "publisher": "Vormetric",
...
### <a name="puppet-enterprise-agent"></a>NUKU Enterprise Agent.
            {
...
### <a name="microsoft-monitoring-agent-for-azure-operational-insights"></a>Microsoft Azure'i töö ülevaated Agent jälgimine
            {
...
### <a name="azure-iaas-antimalware"></a>Azure'i IaaS Ründevaratõrje
          {
...
### <a name="eset-file-security"></a>ESET turvalisus
          {
            "publisher": "ESET",
            "type": "FileSecurity",
            "typeHandlerVersion": "6.0",
...
### <a name="confer-advanced-threat-prevention-and-incident-response-for-azure"></a>Täiustatud ohtu vältimine ja langeva vastuse anda Azure
          {
            "publisher": "Confer",
            "type": "ConferForAzure",
            "typeHandlerVersion": "1.0",
...
### <a name="barracuda-vpn-connectivity-agent-for-microsoft-azure"></a>Microsoft Azure Barracuda VPN ühenduvuse Agent
          {
...
### <a name="alert-logic-log-manager"></a>Teatiste loogika Log haldur
          {
...
Lisateavet selle kohta, kuidas konfigureerida diagnostika leiate [Azure'i diagnostika laiend](virtual-machines-windows-extensions-diagnostics-template.md)
...
Näidetes, asendage versiooninumber numbriga uusim versioon.

Siin on näide täielik VM malli kohandatud skript laiendiga.
